"""Daly profile: central dataset registry.

Single source of truth for dataset-level provenance + descriptions. Each key is
a stable `dataset_id` referenced from credible_sets.py, summary_stats.py, and
coloc.py. New datasets are registered here, not in the product configs.

Fields:
- resource: resource name used in routing (shared across datasets sometimes)
...
- subdataset_id_field: when collection=true, which harmonized-metadata field
  identifies a sub-study (e.g. "phenotype_code")
"""
...
datasets = {
    ...
    "finngen_gwas_r12": {
        ...
        "trait_type": "binary",
        "data_type": "gwas",
        "metadata_file": "gs://daly-genetics-results/mapping_files/finngen_r12_pheno.json",
        "metadata_harmonizer": "finngen_r13",
    },
    "finngen_nmr": {
        "resource": "finngen",
