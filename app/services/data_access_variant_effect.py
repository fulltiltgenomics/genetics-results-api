...
class DataAccessObjectVariantEffect(BaseDataAccessObject):
    """Abstract base class for data access operations for variant_effect data.

    Keyed by ``dataset_id`` (not resource): the Marderstein resource ships two
    distinct predictor files (chrombpnet, flare), each a separate per-dataset entry.
    """
    ...
    async def stream_range(
        ...
        """
        Stream all scored variants in a positional range.
...
class DataAccessFactoryVariantEffect(BaseFactory):
    ...
    def get_config_entry(self, dataset_id: str) -> dict:
        ...
        try:
            return [c for c in variant_effect_data if c["dataset_id"] == dataset_id][0]
        except IndexError:
            raise ValueError(
...
class DataAccessVariantEffect(BaseDataAccess[DataAccessObjectVariantEffect]):
    ...
    def create_factory(self) -> BaseFactory:
        ...
        return DataAccessFactoryVariantEffect()

    async def _get_dataset_access(
        self, dataset_id: str
    ) -> DataAccessObjectVariantEffect:
    ...
    def _select_dataset_ids(resources: List[str]) -> list[str]:
        ...
        return [c["dataset_id"] for c in variant_effect_data if c["resource"] in wanted]
    ...
    async def _stream_datasets(
        ...
        sort_key_fn = create_sort_key(header_with_resource, SORT_CONFIG_VARIANT_EFFECT)
    ...
    async def stream_by_variant(
        ...
        """Stream predicted effects at a variant position (point read at pos).

        With ref/alt given, rows are filtered to the matching allele pair; without
        alleles, every model/context row scored at the position is returned.
        """
    ...
    async def stream_by_region(
        ...
        """Stream all scored variants in the positional range [start, end]."""
        dataset_ids = self._select_dataset_ids(resources)
        return await self._stream_datasets(
    ...
    async def stream_by_gene(
        ...
        gcloud subclass normalizes the chrom to the file's "chr"-prefixed seqname.
        Only the first available gencode version's first position is used (a single
        gene resolves to one locus), keeping a single-chromosome range query.
        """
