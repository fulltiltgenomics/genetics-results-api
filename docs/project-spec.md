...
| gene-groups | HGNC gene-group membership (`GET /gene_group/members?group_id=\|group_name=` returns all descendant member genes by lineage, with symbol/ensembl/coords) and symbol normalization (`GET /gene/normalize?symbols=<comma-separated>` resolves approved/alias/previous symbols to current approved HGNC symbols) |
