...
2. **Tabix-based queries**: Genomic region queries use htslib tabix (subprocess) to fetch data from indexed `.tsv.gz` files on GCS. Exome variant results are served for both genebass (filtered to p < 1e-4) and IBD exome (resource: `ibd_exome_2026`, 3 phenotypes: IBD, UC, CD, exome-wide significant only). Individual trait exome variant results can be queried by phenotype via the `exome_results_by_phenotype` endpoint, which uses prefix/suffix patterns to locate per-trait data files. The IBD exome data includes `n_cases`/`n_controls` columns (nullable, absent for genebass).
