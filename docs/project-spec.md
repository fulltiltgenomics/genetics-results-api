...
- **Startup cache warming**: the FastAPI `lifespan` (which, unlike `run_server.py`'s `__main__` validations, runs inside the uvicorn serving process) concurrently calls `warm_all()` on the credible-set (`DataAccess`), coloc (`DataAccessColoc`), expression (`DataAccessExpression`), and chromatin-peaks (`DataAccessChromatinPeaks`) services. These construct every gcloud tabix data-access object and call its `warm()` to prefetch headers and the `.tbi` index cache (non-blocking), so the first request per resource never pays a blocking `tabix -H` header fetch or a cold `.tbi` download. (Sumstats serves per-phenotype files that aren't enumerable, so it isn't warmed — same reason `verify_all_data_files()` skips them.) Per-file failures are logged and skipped (`verify_all_data_files()` remains the authoritative reachability gate), so a single bad file never aborts startup.
