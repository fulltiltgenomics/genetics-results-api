...
def test_resource_is_appended_to_header_and_every_row():
    files = [{"id": "schema_gene_based", "resource": "schema"}, {"id": "bipex_gene_based", "resource": "bipex"}]
    results = [
    ...
    assert out == [
        ...
        "SCHEMA2\tschizophrenia\tRB1CC1\t10.8\tschema",
        "BipEx2\tbipolar_disorder\tRB1CC1\t11.1\tbipex",
    ]


def test_header_comes_from_the_first_file_with_output():
    files = [{"id": "a", "resource": "genebass"}, {"id": "b", "resource": "ibd"}]
    results = [b"", HEADER + b"IBD_exome\tinflammatory_bowel_disease\tNOD2\t9.2\n"]

    out = asyncio.run(_collect(files, results)).decode().splitlines()

    assert out[0].endswith("\tresource")
    assert out[1].endswith("\tibd")
    assert len(out) == 2
